...
@database_router.post('/database/create', tags=['Database'], response_model=DatabaseSchema, summary='Create a new database record - Crear un nuevo registro en la base de datos.')
def create_new_database_record(database: DatabaseSchema = Depends()):
    try:
        db = SessionLocal()
...
@database_router.get('/database/get', tags=['Database'], response_model=List[DatabaseSchema], summary='Get all the database records - Obtener todos los registros de la base de datos.')
...
@database_router.put('/database/update/{tin_to_search}', tags=['Database'], response_model=UpdateSchema, summary='Update a database record by TIN - Actualizar un registro de la base de datos por NIT.')
def update_database_record_by_tin(tin_to_search: int = Path(..., title='TIN', description='TIN of the record to be updated. - NIT del registro a actualizar.', gt=0), database: UpdateSchema = Depends()):
    try:
        db = SessionLocal()
...
@database_router.delete('/database/delete/{tin}', tags=['Database'], summary='Delete a database record by TIN - Eliminar un registro de la base de datos por NIT.')
def delete_database_record_by_tin(tin: int = Path(..., title='TIN', description='TIN of the record to be deleted. - NIT del registro a eliminar.', gt=0)):
    try:
        db = SessionLocal()
